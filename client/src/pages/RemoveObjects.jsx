  ...
  return (
    ...
    text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
          ...
          <input type="file" accept='image/*' onChange={(e)=>setInput(e.target.files[0])} className='w-full px-3 mt-2 outline-none text-sm 
          rounded-md border border-gray-300 text-gray-600' required/>
          
          <p className='mt-6 text-sm font-medium'>
            Describe object name to remove
          </p>
          <textarea value={object} onChange={(e)=>setObject(e.target.value)} rows={4} className='w-full px-3 mt-2 outline-none text-sm 
